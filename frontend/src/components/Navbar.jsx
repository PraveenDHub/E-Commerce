    ...
    <nav className="sticky top-0 z-50 bg-white/95 backdrop-blur-md shadow-sm border-b border-gray-100 w-full">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="flex items-center justify-between h-16">
          ...
          <Link
            to="/"
            className="flex items-center gap-2 sm:gap-3 text-lg sm:text-2xl font-bold text-blue-600 hover:text-blue-700 transition-colors"
          >
          ...
          <div className="flex items-center gap-2 sm:gap-4">
            ...
            {!isAdminRoute && (
              <form
                ...
                className="hidden sm:flex items-center bg-gray-50 border border-gray-200 focus-within:border-blue-500 rounded-3xl px-4 py-2 w-48 lg:w-72 transition-all"
              >
                <input
                  type="text"
                  placeholder="Search..."
            ...
            {/* Cart */}
            <Link to="/cart" className="relative p-2 sm:p-3 hover:bg-gray-100 rounded-2xl transition-all group">
            ...
            {isAuthenticated ? (
              <div className="relative">
                ...
                >
                  <img src={user.avatar?.url || "/avatar.png"} alt={user.name} className="w-full h-full object-cover" />
                </button>
                  ...
                  <div className="absolute top-14 right-0 w-72 bg-white rounded-3xl shadow-2xl border border-gray-100 py-3 z-50 overflow-hidden">
                    <div className="px-6 py-5 border-b border-gray-100 bg-gray-50">
                      <div className="flex items-center gap-4">
                        <img src={user.avatar.url} alt={user.name} className="w-12 h-12 rounded-2xl object-cover ring-2 ring-white" />
                        <div>
                    ...
                    <div className="py-2">
                      <Link to="/profile" className="flex items-center gap-4 px-6 py-3.5 text-gray-700 hover:bg-gray-50 transition-colors" onClick={() => setIsProfileMenuOpen(false)}>
                        <UserCircle className="w-5 h-5" /> <span>My Profile</span>
                      </Link>
                      <Link to="/orders" className="flex items-center gap-4 px-6 py-3.5 text-gray-700 hover:bg-gray-50 transition-colors" onClick={() => setIsProfileMenuOpen(false)}>
                    ...
                    <button onClick={handleLogout} className="flex items-center gap-4 w-full px-6 py-3.5 text-red-600 hover:bg-red-50 transition-colors font-medium">
              ...
              <div className="hidden sm:flex items-center gap-3">
                <Link to="/login" className="px-5 py-2 text-sm font-medium text-gray-700 hover:bg-gray-100 rounded-2xl transition-all">Login</Link>
                <Link to="/register" className="px-6 py-2 text-sm font-medium bg-blue-600 text-white hover:bg-blue-700 rounded-2xl transition-all">Register</Link>
              </div>
            )}

            {/* Mobile Menu Button */}
            <button onClick={() => setIsMenuOpen(!isMenuOpen)} className="md:hidden p-2 hover:bg-gray-100 rounded-2xl transition-colors">
        ...
        <div className="px-6 py-6 bg-white border-t space-y-6 text-lg font-medium">
          ...
          {!isAuthenticated && (
            <div className="flex gap-4 pt-2">
                <Link to="/login" onClick={() => setIsMenuOpen(false)} className="flex-1 text-center py-3 bg-gray-100 rounded-xl">Login</Link>
                <Link to="/register" onClick={() => setIsMenuOpen(false)} className="flex-1 text-center py-3 bg-blue-600 text-white rounded-xl">Register</Link>
            </div>
          )}
          {isAuthenticated && user?.role === "admin" && !isAdminRoute && (
            <button onClick={() => {navigate("/admin/dashboard"); setIsMenuOpen(false);}} className="flex w-full items-center justify-center gap-3 px-6 py-3 bg-zinc-900 hover:bg-black text-white font-medium rounded-2xl border border-zinc-700">
              <ShieldCheck className="w-5 h-5 text-violet-400" /> Admin Dashboard
            </button>
          )}
        </div>
      </div>
